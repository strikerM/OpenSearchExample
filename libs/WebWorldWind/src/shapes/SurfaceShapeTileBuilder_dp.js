...
define([
    ...
    function (ArgumentError,
        ...
        SurfaceShapeTileBuilder.prototype.doDeepPickingRender_original = function (dc) {
        ...
        SurfaceShapeTileBuilder.prototype.doDeepPickingRender = function (dc) {
            var idxTile, lenTiles, idxShape, lenShapes, idxPick, lenPicks, po, shape, tile;
            var mousePosition = dc.mousePickPosition;
            ...
            for (idxShape = 0, lenShapes = deepPickShapes.length; idxShape < lenShapes; idxShape += 1) {
                ...
                for (var k = 0; k < contours.length; k++) {
                    var polygons = contours[k].polygons;
                    for (var kk = 0; kk < polygons.length; kk++) {
                        var polygon = polygons[kk];
                        isInside = this.pointInPolygon(mousePosition, polygon);
                        if (isInside) {
                            po = new PickedObject(shape.pickColor.clone(),
                                shape.pickDelegate ? shape.pickDelegate : shape, null, shape.layer, false);
                            resolvablePickObjects.push(po);
        ...
        SurfaceShapeTileBuilder.prototype.pointInPolygon = function (point, contour) {
            var inside = false;
            var x = point.latitude;
            var y = point.longitude;

            for (var i = 0, j = contour.length - 1; i < contour.length; j = i++) {
                var xi = contour[i].latitude;
                var yi = contour[i].longitude;
                var xj = contour[j].latitude;
                var yj = contour[j].longitude;
